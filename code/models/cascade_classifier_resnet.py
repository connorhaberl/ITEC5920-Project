...
class cascade_classifier_resnet(ClassificationModel):
    ...
    def predict(self, X):
        ...
        for i in range(rf_predictions.shape[0]):
            strongest_classification =0
            strongest_index = 0 
            for j in range(5):
                if rf_predictions[i][j]>strongest_classification:
                   strongest_classification = rf_predictions[i][j]
                   strongest_index = j
            single_col[i] = strongest_index
